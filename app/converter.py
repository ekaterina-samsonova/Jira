...
def _replace_view_in_browser(html: str) -> tuple[str, bool]:
    ...
    patterns = [
        (
            r"(Если данное письмо отображается некорректно[^<]*)(?:нажмите\s+)?(сюда)",
            rf"\1{link}",
        ),
        (
            r"(Если\s+(?:данное\s+)?письмо[^<]*?)(<a[^>]*>[^<]*</a>|сюда)",
            rf"\1{link}",
        ),
    ]
    for pattern, repl in patterns:
        updated, count = re.subn(pattern, repl, html, count=1, flags=re.IGNORECASE)
        if count:
            return updated, True
    if "%%view_email_url%%" not in html and "отображается некорректно" in html.lower():
        updated = re.sub(
            r"(отображается некорректно[^<]*?)(сюда)",
            rf"\1{link}",
            html,
            count=1,
            flags=re.IGNORECASE,
...
def _replace_or_insert_privacy(html: str, privacy_url: str) -> tuple[str, bool]:
    if not privacy_url:
        return html, False
    block = block4_privacy(privacy_url)
    ...
    docsfera_links = re.findall(r"https?://docsfera\.ru/[^\s\"'<>]+", html, re.IGNORECASE)
    privacy_candidates = [
        u
        for u in docsfera_links
        if "unsubscribe" not in u.lower() and "voting/cxq" not in u.lower()
    ]
    chosen = privacy_url or (privacy_candidates[0] if privacy_candidates else "")
    if not chosen:
        return html, False
...
def convert_mindbox_to_sfmc(html: str, params: ConversionParams) -> ConversionResult:
    ...
    if ok:
        changes.append("Обновлен блок Privacy Link (docsfera.ru)")
    elif params.privacy_url:
        warnings.append("Privacy URL указан, но блок не удалось вставить автоматически")

    if params.utm_campaign and params.cxq_brand:
        ...
        warnings.append("Блок №5: укажите utm_campaign и Brand для генерации CXQ-ссылок")
